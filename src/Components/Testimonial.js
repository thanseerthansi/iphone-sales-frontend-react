...
import { MdPhoneIphone,MdOutlineFormatColorFill, MdPlace } from 'react-icons/md';
...
export default function Testimonial() {
    ...
    const gettestimonial=async()=>{
      ...
      const addtestimonial=async(e)=>{
        e.preventDefault(); 
        accesscheck()
        let datalist={"name":username,"place":place,"rating":rating,"review":review}
        // console.log("datalist",datalist)
        if(edittestimonial){
            // console.log("dfsdffs",editstatus)
            datalist.id=edittestimonial.id
        }
        // console.log("dataaaaaaaaaaalist",datalist)
        let data = await Callaxios("post","product/testimonial/",datalist)
        // console.log("dataresponse",data)
        if (data.data.Status===200){
            if(edittestimonial){
                notify("Updated Successfully")
            }else{notify("Added Successfully")}
            setallnull()
    ...
    const functionedit=(itm)=>{
        setedittestimonial(itm)
        setusername(itm.name)
        setplace(itm.place)
        setrating(itm.rating)
        setreview(itm.review)
        setcartmodal(!cartmodal)
    }
    const deletefunction = async(itmid,k)=>{
        accesscheck()
        let datalist = {"id":JSON.stringify([itmid])}
        let data = await Callaxios("delete","product/testimonial/",datalist)
        console.log("data",data)
        if(data.data.Status===200){
           let splc = testimonialdata
           splc.splice(k,1)
           settestimonialdata(() => [ ...splc]);
        ...
        <div className='md:col-span-1'>
            ...
            :null} 
                ...
                <ToastContainer />
                    ...
                    <b className='text-red-600 '>Testimonial</b>
                    {/* search start */}
                        ...
                        <section className="relative md:py-10 py-16  bg-white dark:bg-slate-900">
                            <div className="container">
                                ...
                                <div className="lg:col-span-12 md:col-span-12">
                                    ...
                                    <form onSubmit={(e)=>addtestimonial(e)}   className='pt-5' >
                                        ...
                                        <div className="grid lg:grid-cols-12 lg:gap-6">
                                            ...
                                            <div className="text-left">
                                                <label htmlFor="name" className="font-semibold">User Name: <b className='text-red-600'>*</b></label>
                                                ...
                                                <input required  type='text' onChange={(e)=>setusername(e.target.value)} value={username}  className='form-input pl-11' placeholder='UserName'/>
                                                </div>
                                            </div>
                                            </div>
                                            <div className="lg:col-span-6 mb-5">
                                            <div className="text-left">
                                                <label htmlFor="email" className="font-semibold">Place<b className='text-red-600'>*</b></label>
                                                <div className="form-icon relative mt-2">
                                                <i className="w-4 h-4 absolute top-3 left-4"><BiText  size={18} /></i>
                                                <input  required  onChange={(e)=>setplace(e.target.value)} value={place} className='form-input pl-11' placeholder='Place'/>
                                                </div>
                                            </div>
                                            </div>
                                        </div>
                                        <div className="grid lg:grid-cols-12 lg:gap-6">
                                            <div className="lg:col-span-6 mb-5">
                                            <div className="text-left">
                                                <label htmlFor="contact" className="font-semibold">Rating <b className='text-red-600'>*</b></label>
                                                <div className="form-icon relative mt-2 flex border ">
                                                <i className="w-4 h-4 absolute top-3 left-4"><MdOutlineFormatColorFill size={18} /></i>
                                                {/* <input   name="color" id="color" type="color" className="form-input pl-11" placeholder="color" /> */}
                                                <ReactStars 
                                                classNames="ml-[60px]"
                                                key={`stars_${rating}`}
                                            ...
                                            <div className="lg:col-span-6 mb-5">
                                            <div className="text-left">
                                                <label htmlFor="email" className="font-semibold">Review<b className='text-red-600'>*</b></label>
                                                <div className="form-icon relative mt-2">
                                                <i className="w-4 h-4 absolute top-3 left-4"><BiText  size={18} /></i>
                                                <textarea required  onChange={(e)=>setreview(e.target.value)} value={review}  className='form-input pl-11' placeholder='Review'/>
                                                </div>
                                            </div>
                                            </div>
                                        </div>
                            ...
                            <div className="modal-footer flex flex-shrink-0 flex-wrap items-center justify-end p-4 border-t border-gray-200 rounded-b-md">
                                    <button onClick={()=>setcartmodal(!cartmodal) & setallnull()}  type="button" className="inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-purple-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-purple-800 active:shadow-lg transition duration-150 ease-in-out" data-bs-dismiss="modal">
                                        Close
                                    </button>
